...
// Helper functions for API key encoding/decoding
...
const decodeApiKey = (encoded: string): string => {
  ...
  try {
    ...
    // Validate that the decoded key looks like a valid API key
    return decoded;
  } catch {
    // If decoding fails, assume it's already a plain text key
    return encoded.trim();
  }
};
...
const App: React.FC = () => {
  ...
  const [creditText, setCreditText] = useState<string>("");
  const [stationDescription, setStationDescription] = useState<string>(
    "The ultimate interactive AI radio station."
  );
  ...
  const handleBroadcast = async () => {
    ...
    try {
      // Decode API key if provided and validate it's not empty
      let decodedApiKey: string | undefined = undefined;
      if (geminiApiKey && geminiApiKey.trim()) {
        const decoded = decodeApiKey(geminiApiKey);
        // Only use the decoded key if it's not empty and looks valid
        if (decoded && decoded.trim() && decoded.length > 10) {
          decodedApiKey = decoded;
        }
      }
      const { rawBytes, duration, audioContext, source } =
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#050505] text-white font-sans selection:bg-white/20">
      ...
      >
        <div className="p-6 md:p-8 pb-4 flex items-center justify-between min-w-[320px]">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-blue-600 rounded flex items-center justify-center font-black text-white shadow-lg shadow-blue-600/20">
              S
            </div>
            <h1 className="text-xl font-black italic tracking-tighter uppercase truncate">
              {stationName}
            </h1>
          </div>
          <button
        ...
        <nav className="px-4 space-y-1 mb-8 min-w-[320px]">
          ...
          >
            <svg
              ...
              strokeLinejoin="round"
            >
              <path d="M2 12a10 10 0 0 1 10-10c1.8 0 3.5.5 5 1.4L18.4 2a1 1 0 0 1-1.6.8v6.2a1 1 0 0 1-1 1h-6.2a1 1 0 0 1-.8-1.6L14.6 6.8A6 6 0 1 0 18 12h2a8 8 0 1 1-18 0Z" />
            </svg>
            Broadcast Console
          </button>
          <button
          ...
          {VOICES.map((voice) => (
            ...
            >
              ...
              <div className="text-left overflow-hidden">
                ...
                <div className="text-[10px] text-white/40 truncate w-32">
                  {voice.description}
                </div>
              </div>
              {selectedVoice.id === voice.id && (
      ...
      <main className="flex-1 h-full flex flex-col relative overflow-hidden transition-all duration-300">
        ...
        {activeTab === "broadcast" && (
          <div className="flex-1 flex flex-col h-full relative p-4 md:p-6 lg:p-8">
            <div
              className={`flex-1 flex flex-col xl:flex-row gap-4 md:gap-6 lg:gap-8 min-h-0 ${
                isAlbumFullscreen ? "justify-center items-center" : ""
              }`}
            >
              {/* Layout 1: Teleprompter / Script Area */}
              <div
                className={`glass rounded-[1.5rem] md:rounded-[2.5rem] flex-col p-5 md:p-10 relative overflow-hidden transition-all duration-500 ${
                  isAlbumFullscreen ? "hidden" : "flex flex-[3]"
                } ${isBroadcasting ? "min-h-[160px] xl:min-h-0" : "flex-1"}`}
              >
                  ...
                  <div className="absolute inset-0 z-[70] flex flex-col items-center justify-center bg-black/60 backdrop-blur-2xl transition-all duration-500 p-8 text-center">
                    <div className="relative w-48 h-48 md:w-64 md:h-64 flex items-center justify-center">
                      ...
                      <div
                        ...
                        style={{
                          borderColor: `${selectedVoice.color} transparent transparent transparent`,
                        }}
                      ></div>
                    ...
                    <div className="mt-8 space-y-2">
                      <div className="font-mono text-[9px] text-white/30 uppercase tracking-[0.4em] animate-pulse">
                        Neural Signal Synthesis
                      </div>
                      <h2 className="text-[10px] md:text-xs font-bold tracking-[0.2em] uppercase text-white h-4">
                        {loadingMessage}
                      </h2>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="font-mono text-[9px] md:text-[10px] text-white/40 flex gap-4 uppercase tracking-widest">
                      ...
                      <span className="hidden sm:inline">
                        Signal: <span className="text-white">PCM_24K</span>
                      </span>
                    </div>
                  </div>
                  {isBroadcasting && (
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] md:text-[10px] font-mono text-red-500 animate-pulse font-bold tracking-tighter">
                        LIVE
                      </span>
                ...
                <div className="flex-1 relative">
                  {isBroadcasting ? (
                    <div className="absolute inset-0 flex flex-col">
                      ...
                      <div
                        ref={scrollRef}
                        className="flex-1 overflow-y-auto no-scrollbar py-[10vh] md:py-[25vh]"
                      >
                            ...
                            .map((para, i) => (
                              <p
                                ...
                                style={{
                                  ...
                                  textShadow: `0 0 50px ${selectedVoice.color}44`,
                                }}
                              >
                                {para}
                              </p>
              ...
              <div
                className={`flex flex-col gap-4 transition-all duration-700 ${
                  isAlbumFullscreen ? "w-full h-full flex-1" : "flex-[2]"
                } ${isBroadcasting ? "h-auto" : ""}`}
              >
                <div
                  className={`glass rounded-[1.5rem] md:rounded-[2.5rem] flex flex-col items-center justify-center p-4 md:p-6 relative overflow-hidden group transition-all duration-700 ${
                    isAlbumFullscreen
                      ? "flex-1 p-8 md:p-16"
                      : isBroadcasting
                      ? "py-6 md:py-8"
                ...
                >
                  ...
                  <div className="relative shrink-0 transition-transform duration-700">
                    {/* Outer Vinyl / CD */}
                    <div
                      className={`rounded-full bg-black shadow-2xl flex items-center justify-center relative overflow-hidden border-2 md:border-4 border-white/5 animate-spin-slow transition-all duration-700 ${
                        !isBroadcasting ? "paused" : ""
                      } ${
                        isAlbumFullscreen
                          ? "w-48 h-48 sm:w-64 sm:h-64 md:w-80 md:h-80 lg:w-[450px] lg:h-[450px]"
                          : "w-20 h-20 sm:w-32 sm:h-32 md:w-48 md:h-48 lg:w-56 lg:h-56"
                      }`}
                    >
                      ...
                      <div
                        className={`rounded-full overflow-hidden border-2 md:border-4 border-black z-10 shadow-lg transition-all duration-700 ${
                          isAlbumFullscreen
                            ? "w-24 h-24 sm:w-32 sm:h-32 md:w-48 md:h-48 lg:w-56 lg:h-56"
                            : "w-10 h-10 sm:w-16 sm:h-16 md:w-24 md:h-24 lg:w-28 lg:h-28"
                        }`}
                      >
                        <img
                          src={coverImage}
                      ...
                      <div
                        className={`bg-black rounded-full border-1 md:border-2 border-white/20 z-20 transition-all duration-700 ${
                          isAlbumFullscreen
                            ? "w-4 md:w-10 h-4 md:h-10"
                            : "w-1.5 md:w-4 h-1.5 md:h-4"
                        }`}
                      ></div>
                    </div>
                  ...
                  {/* Metadata Bar */}
                  <div
                    className={`text-center w-full flex flex-col items-center justify-center transition-all duration-500 flex-1`}
                  >
                    <span
                      className={`hidden lg:block font-mono text-white/20 uppercase tracking-[0.3em] mb-1 font-bold ${
                        isAlbumFullscreen
                          ? "text-[12px] md:text-[14px] mb-4"
                          : "text-[9px]"
                      }`}
                    >
                    ...
                    <div
                      className={`w-full flex flex-col items-center justify-center transition-all duration-700 ${
                        isAlbumFullscreen ? "gap-2" : "lg:gap-0.5"
                      }`}
                    >
                      {/* Title - Stacks vertically in fullscreen and desktop always */}
                      <h3
                        className={`font-black uppercase tracking-tighter leading-tight whitespace-nowrap transition-all duration-700 ${
                          isAlbumFullscreen
                      ...
                      {/* Credit Text - Block level stack with different style */}
                      {creditText && (
                        <p
                          className={`font-serif italic text-white/60 text-center leading-relaxed transition-all duration-700 mt-1 ${
                            isAlbumFullscreen
                              ? "text-base sm:text-lg md:text-xl lg:text-3xl max-w-2xl"
                              : "text-[10px] sm:text-xs md:text-sm lg:text-base max-w-xs"
                          }`}
                        >
                          {creditText}
                        </p>
                      ...
                      <h4
                        className={`font-mono text-white/40 uppercase tracking-[0.1em] sm:tracking-[0.2em] lg:tracking-[0.25em] font-bold whitespace-nowrap transition-all duration-700 ${
                          isAlbumFullscreen
                            ? "text-xs sm:text-sm md:text-lg lg:text-2xl mt-2"
                            : "text-[9px] sm:text-[10px] md:text-[12px] lg:text-[11px]"
                        }`}
                      >
                        {stationName}
                      </h4>
                      ...
                      <div
                        className={`flex items-center gap-1.5 mt-2 lg:mt-3 px-2 py-0.5 lg:px-3 lg:py-1 rounded-full bg-white/5 border border-white/10 shrink-0 transition-all duration-700 ${
                          isAlbumFullscreen ? "scale-150 mt-8" : ""
                        }`}
                      >
                        <span
                          ...
                          style={{ backgroundColor: selectedVoice.color }}
                        ></span>
                        <span className="text-[8px] sm:text-[9px] font-bold text-white/60 uppercase tracking-widest">
                          {selectedVoice.name}
                        </span>
                      </div>
                    </div>
                    ...
                    <div
                      className={`hidden lg:block flex-1 w-full overflow-y-auto custom-scroll mt-3 px-2 max-w-[280px] transition-all duration-700 ${
                        isAlbumFullscreen ? "hidden" : ""
                      }`}
                    >
                ...
                <div
                  className={`glass rounded-[1rem] md:rounded-[1.25rem] p-3 md:p-4 flex-col gap-3 shrink-0 transition-all duration-500 ${
                    isAlbumFullscreen ? "hidden" : "flex"
                  }`}
                >
                  <Visualizer
                    analyser={analyser}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={
                        !lastAudioBlob || isBroadcasting || isSynthesizing
                      }
                    ...
                    <button
                      ...
                      disabled={
                        isBroadcasting || isSynthesizing || !inputText.trim()
                      }
          ...
          <div className="flex-1 flex flex-col items-center justify-start p-4 md:p-8 lg:p-12 overflow-y-auto custom-scroll">
            <div className="w-full max-w-2xl glass rounded-[2rem] md:rounded-[3rem] p-6 md:p-14 text-center space-y-6 md:space-y-10 my-4">
              <div className="space-y-2">
                ...
                <p className="text-white/40 font-medium text-xs md:text-sm">
                  Customize your broadcast visuals and station ID.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 text-left">
                <div className="space-y-1">
                  <label className="text-[8px] md:text-[10px] font-bold text-white/30 uppercase tracking-widest ml-1">
                    Station Name
                  </label>
                  <input
                    type="text"
              ...
              <div className="space-y-1 text-left">
                ...
                <textarea
                  ...
                  placeholder="e.g., Produced by John Doe | Mixed at Studio X"
                  rows={3}
                />
              </div>
              ...
              <div className="space-y-1 text-left">
                ...
                <div className="relative">
                  <input
                    ...
                    onBlur={(e) => {
                      // Auto-encode the API key when user finishes typing
                      const value = e.target.value.trim();
                      if (value && value.length > 0) {
                        try {
                          // Try to decode - if successful, it's already encoded
                          const decoded = atob(value);
                          // If decoded successfully and looks different from original, it's encoded
                          if (decoded !== value) {
                            return; // Already encoded, do nothing
                          }
                        } catch {
                          // Decoding failed, so it's plain text - encode it
                        }
                        // Encode the plain text API key
                        setGeminiApiKey(encodeApiKey(value));
                      }
                    }}
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-12 text-xs md:text-sm font-mono focus:outline-none focus:border-white/30 transition-all"
                ...
                <p className="text-[8px] md:text-[9px] text-white/20 ml-1 mt-1">
                  {geminiApiKey
      ...
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden">
        <div
          ...
          style={{ backgroundColor: selectedVoice.color }}
        ></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-600/5 blur-[150px] rounded-full"></div>
      </div>
    </div>
  );
};
